...
\section{Strona internetowa}
	...
	\subsection{Responsive design}
		...
		\begin{lstlisting}[label=lst:flexLayout, caption=Menu nawigacji zależne od rozmiaru ekranu, float=h]
			<div fxHide.gt-xs>
...
\section{GraphQL API}
	...
	\subsection{Warstwa Api}
		...
		\subsubsection*{Middleware}
			...
			\begin{lstlisting}[label=lst:middleware, caption=Middleware zapisujące użytkowników, float=h]
...
\section{Serwis uwierzytelniający}
	...
	\subsection{Autoryzacja dostępu do API}\label{sec:oidApi}
		...
		\begin{lstlisting}[label=lst:openIdService, caption=Serwis HTTP pobierający dane użytkownika, float=h]
...
\section{Odwrócone proxy}
	...
	\begin{lstlisting}[label=lst:mainProx, caption=Ustawienia proxy, float=h]
		upstream spa {
			server          spa:4200;
		}
		upstream reserver {
	...
	\begin{lstlisting}[label=lst:authProx, caption=Proxy auth, float=h]
		upstream auth {
			server          authServer;
		}
